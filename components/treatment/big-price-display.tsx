    ...
    <div className="text-center py-2 animate-in fade-in zoom-in duration-300">
      {option.isPromo ? (
        <div className="flex flex-col items-center justify-center">
          ...
          {option.promoEnds && (
            <span className="inline-block mt-2 text-xs text-foreground uppercase tracking-widest font-semibold bg-highlight/20 border border-highlight/40 px-3 py-1 rounded-full">
              Offer ends {option.promoEnds}
            </span>
          )}
        </div>
      ) : (
        <p className="text-4xl text-foreground font-bold drop-shadow-md">
          {option.price}
        </p>
      )}
    </div>
