...
export const selectTriggerVariants = cva(
  // Layout & Accessibility (Structural)
  "relative inline-flex min-h-9 w-full min-w-36 select-none items-center justify-between gap-2 rounded-lg border px-3 text-left transition-all outline-none pointer-coarse:after:absolute pointer-coarse:after:size-full pointer-coarse:after:min-h-11 sm:min-h-8 sm:text-sm",
  {
    variants: {
      variant: {
        // Homogenized Surface (Colors & Focus)
        default:
...
export function SelectButton({
  ...
  const defaultProps = {
    children: (
      <>
        <span className="flex-1 truncate in-data-placeholder:text-muted-foreground/70">
          {children}
        </span>
    ...
    <SelectPrimitive.Portal>
      ...
      >
        ...
        >
          ...
          <div className="relative h-full min-w-(--anchor-width) rounded-xl border border-border bg-popover text-popover-foreground shadow-xl backdrop-blur-md">
...
export function SelectItem({
  ...
  return (
    <SelectPrimitive.Item
      className={cn(
        "grid min-h-8 in-data-[side=none]:min-w-[calc(var(--anchor-width)+1.25rem)] cursor-default grid-cols-[1rem_1fr] items-center gap-2 rounded-sm py-1 ps-2 pe-4 text-base outline-none data-disabled:pointer-events-none cursor-pointer transition-colors data-highlighted:bg-accent data-highlighted:text-accent-foreground data-disabled:opacity-64 sm:min-h-7 sm:text-sm [&_svg:not([class*='size-'])]:size-4.5 sm:[&_svg:not([class*='size-'])]:size-4 [&_svg]:pointer-events-none [&_svg]:shrink-0",
        className,
      )}
...
export function SelectGroupLabel(
  ...
  return (
    <SelectPrimitive.GroupLabel
      className="px-2 py-1.5 font-bold text-muted-foreground text-[10px] uppercase tracking-widest"
